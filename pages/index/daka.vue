					...
					<view class="uni-init">
						...
						<view class="uni-init-con">
							...
							<view class="init-content">
								...
								<video :src="items.Video" controls="controls" :style="{display: items.Video == '' ? 'none' : 'block'}" direction="0"></video>
							</view>
							<view class="luyin" v-show="items.Music != ''">
								<text>录音音频</text>
								<view>
									<image src="../../static/bofan_03.png" mode="" v-if="inList[index] == true" @tap="startRecord(items.Music, index)"></image>
									<image src="../../static/bofan_04.png" mode="" v-if="inList[index] == false" @tap="stopRecord(index)"></image>
								</view>
							</view>
							<view class="dizhi" :style="{display: (items.position == null || items.position == '') ? 'none' : 'block'}">{{items.position}}</view>
						</view>
					</view>
					<!-- 图标 -->
	...
	export default {
		...
		methods: {
			...
			qz(type) {
				...
				uni.request({
					url: helper.websiteUrl + 'circle/getbycateid',
					method: 'POST',
					data: helper.postdata({
						userid: helper.getstate().userid,
						cateid: this.cateid,
			...
			actionSheetTap() {
				...
				uni.showActionSheet({
					...
					success: (e) => {
						uni.navigateTo({
							url: 'dakariji?Id=' + this.qzinfo.Id
						});
					}
				})
			}
