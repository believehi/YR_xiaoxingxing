		...
		<view class="container">
			...
			<view class="main">
				...
				<view class="main-zp" v-show="activeIndex == 1">
					<view class="uni-item" v-for="(item6, index6) in topiclist" :key="index6">
						<view>
							<video controls :src="item6.Video" :id="item6.Id" @fullscreenchange="fullchange($event, index6)"></video>
							<image :src="item6.pics" @tap="bofan(item6.Id, index)"></image>
						</view>
					</view>
				</view>
				<!-- 说说 -->
				...
				uni.request({
					...
					success: res => {
						if (res.data.code == 200) {
							...
							{
								this.baseinfo.Headimgurl = helper.imgurl + this.baseinfo.Headimgurl;
							}
